...
def draw_paths_horizontal_backup(times, paths, N, expectations, title=None, KDE=False, marginal=False, marginalT=None,
                                 envelope=False,
                                 lower=None, upper=None, style="seaborn-v0_8-whitegrid", colormap="RdYlBu_r", **fig_kw):
    with plt.style.context(style):
        if marginal:
...
def draw_paths_horizontal(times, paths, N, expectations, title=None, KDE=False, marginal=False, marginalT=None,
    ...
    with plt.style.context(style):
        if marginal:
            ...
            ax1.plot(times, expectations, '--', lw=1.75, label='$E[X_t]$')
            if envelope:
                ax1.fill_between(times, np.sqrt(upper), np.sqrt(lower), alpha=0.25, color='grey')
            plt.subplots_adjust(wspace=0.025, hspace=0.025)
